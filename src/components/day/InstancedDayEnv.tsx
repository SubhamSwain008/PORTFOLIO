...
export function InstancedDayTrees({ items }: { items: EnvItem[] }) {
    const configs = [
        { baseColor: "#2a6a30", shape: "sphere", height: 2.0, radius: 1.2 },
        { baseColor: "#256b2d", shape: "cone", height: 2.8, radius: 1.1 },
        { baseColor: "#5a7a2a", shape: "sphere", height: 1.8, radius: 1.4 },
        { baseColor: "#3a7a40", shape: "sphere", height: 2.4, radius: 1.0 },
        { baseColor: "#4a8a35", shape: "cone", height: 2.2, radius: 1.3 },
    ];
    ...
    const trunkMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: "#5a3e28", roughness: 0.85, metalness: 0, flatShading: true }), []);
    const leafMaterial = useMemo(() => new THREE.MeshStandardMaterial({ map: leafTex, roughness: 0.75, metalness: 1, flatShading: true, vertexColors: true }), [leafTex]);
...
export function InstancedDayFencePerimeter() {
    ...
    const spacing = 4 / picketCountPerSegment;
    ...
    useEffect(() => {
        ...
        const addSegment = (x: number, y: number, z: number, rotationY: number) => {
            ...
            for (let i = 0; i < picketCountPerSegment; i++) {
                const px = -4 / 2 + spacing / 2 + i * spacing;
    ...
    const railMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: "#8a7060", roughness: 0.8, metalness: 0.02 }), []);
    const picketMaterial = useMemo(() => new THREE.MeshStandardMaterial({ color: "#7a6455", roughness: 0.82, metalness: 0.02 }), []);
